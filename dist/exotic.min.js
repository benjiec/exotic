...
alert("Not configured to fetch value by samples and observations.");cb([]);}});'use strict';function ExoticController($scope,$http){$scope.samples=[];$scope.observations=[];$scope.properties=[];$scope.selected_samples=[];$scope.selected_observations=[];$scope.fetches=[];$scope.example_value=undefined;$scope.data_by_sample={};$scope.fields=[];function update_fields(){if($scope.example_value!==undefined){$scope.fields=[];for(var i=0;i<$scope.selected_observations.length;i++){for(var j=0;j<$scope.example_value.__fields__.length;j++){$scope.fields.push([$scope.selected_observations[i].id,$scope.example_value.__fields__[j]]);}}}}
function update_properties(){for(var i=0;i<$scope.samples.length;i++){var sample=$scope.samples[i];var properties=sample.properties();for(var j=0;j<properties.length;j++){if($scope.properties.indexOf(properties[j])<0){$scope.properties.push(properties[j]);}}}}
...
update_fields();}
exoticSamples.get($http,function(samples){$scope.samples=samples;update_properties();});exoticObservations.get($http,function(observations){$scope.observations=observations;});}
